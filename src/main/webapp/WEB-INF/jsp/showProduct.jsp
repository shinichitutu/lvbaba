...
<div id="wrapper">
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${requestScope.products}" var="product" varStatus="i">
                <tr>
                    ...
                    <td><input type="button" value="删除产品" class="btn btn-secondary"
                               onclick="location.href='deleteProduct.do?productId=${product.productId}'"/></td>
                    <td><input type="button" value="修改产品" class="btn btn-secondary"
                               onclick="updateProduct('${product.productId}','${product.productName}',
                                       '${product.d_area.country}-${product.d_area.city}','${product.a_area.country}-${product.a_area.city}','${product.limLow}',
                                       '${product.limUp}','${product.days}','${product.hotelId}','${product.productFee}','${product.arrAreaId}','${product.files.filePath}')"/>
                    </td>
    ...
    <div style="text-align: center">
        <input type="button" value="增加产品" id="d1" class="btn btn-primary">
    ...
    <div style="margin-left: 50px;display: none;" id="d2">
        <form action="addProductInfo.do" method="post" enctype="multipart/form-data">
            ...
            <p>人数下限：<input required type="number" min="1" max="5" step="1" name="limLow" class="limLow"
                           placeholder="请选择人数下限"></p>
            <p>人数上限：<input required type="number" min="5" max="30" step="1" name="limUp" class="limUp"
                           placeholder="请选择人数上限"></p>
            <p>行程天数：<input required type="number" min="1" max="10" step="1" name="days" class="days"
                           placeholder="请选择行程天数"></p>
            <p>入住酒店：<select required class="hotelId" name="hotelId">
                <option value="0">--请选择--</option>
            </select><span>选择了目的地之后，才能看到酒店</span></p>
            <p>基础团费：<input required type="number" min="500" max="10000" step="500" name="productFee" class="productFee"
                           placeholder="请输入基本团费"></p>
